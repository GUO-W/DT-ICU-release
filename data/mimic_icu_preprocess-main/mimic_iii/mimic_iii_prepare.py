...
def prepare_mimic_data(mimic_data_dir):
    # Get all ICU stays.
    icu = pd.read_csv(mimic_data_dir+'ICUSTAYS.csv.gz', usecols=['SUBJECT_ID', 'HADM_ID', 'ICUSTAY_ID', 'INTIME', 'OUTTIME'])
    ...
    ch = []
    for chunk in tqdm(pd.read_csv(mimic_data_dir+'CHARTEVENTS.csv.gz', chunksize=100000000,
    ...
    mask = ch_bp.ITEMID.isin(dbp)
    if mask.any():
        ch_bp.loc[mask, 'NAME'] = 'DBP'


    mask = ch_bp.ITEMID.isin(sbp)
    if mask.any():
        ch_bp.loc[mask, 'NAME'] = 'SBP'

    mask = ch_bp.ITEMID.isin(mbp)
    if mask.any():
        ch_bp.loc[mask, 'NAME'] = 'MBP'
    ...
    mask = ch_gcs.ITEMID.isin(gcs_eye + gcs_motor + gcs_verbal)
    if mask.any():
        ch_gcs.loc[mask, 'NAME'] = 'GCS'
    else:
        print("No matching ITEMID found.")
    ...
    oe = pd.read_csv(mimic_data_dir+'OUTPUTEVENTS.csv.gz', usecols = ['ICUSTAY_ID', 'HADM_ID', 'ITEMID', 'CHARTTIME', 'VALUE', 'VALUEUOM'])
    ...
    ie_cv = pd.read_csv(mimic_data_dir+'INPUTEVENTS_CV.csv.gz',
    ...
    ie_mv = pd.read_csv(mimic_data_dir+'INPUTEVENTS_MV.csv.gz',
