...
class MimicDataset(Dataset):
    ...
    def process_file(self, file_path):  # Function to process a single file
        ...
        demo = demo[["gender", "race", "insurance", "anchor_age", "admission_type", "icu_type"]].values.reshape(-1) # iv weight, iii language, marital status
    ...
    def get_dynamic_data(self, dyn):
        keys = ["inputevents", "outputevents", "chartevents"]
        for key in keys:
            if key == "inputevents":
...
def pad_to_bucket_max(batch, boundaries=[0], pad_value=0.0, n_temporal=3):
    ...
    # ---------- STEP 4: last non-padding index per sample -------
    # 0-based; -1 if sequence length is 0
    last_nonpad_idx = torch.tensor(
        [L - 1 if L > 0 else -1 for L in T_lengths],
        dtype=torch.int64
    )

    attention_mask = attention_mask.to(torch.bool)
    # ---------- RETURN in original order + mask -----------------
    return (*padded_tensors, *rest, attention_mask, buckets, last_nonpad_idx)
