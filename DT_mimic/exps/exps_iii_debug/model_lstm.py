...
class DTmodel(nn.Module): # Transformer our model v1.0
    def __init__(
        ...
        # self.conv0 = nn.Conv1d(self.atten_embed_size, self.atten_embed_size, 9, 1, 9 // 2)
        # self.activation = nn.ReLU()
        # self.conv1 = nn.Conv1d(self.atten_embed_size, self.atten_embed_size, 9, 1, 9 // 2)
        # self.conv2 = nn.Conv1d(self.atten_embed_size, self.atten_embed_size, 9, 3, 0)
        # self.conv3 = nn.Conv1d(self.atten_embed_size, self.atten_embed_size, 9, 3, 0)
        # self.pool = nn.AdaptiveAvgPool1d(1)

        self.lstm = nn.LSTM(input_size=self.atten_embed_size, hidden_size=512, num_layers=4, batch_first=True)
        ...
        self.fc0 = nn.Linear(self.atten_embed_size + 512, self.atten_embed_size)
        self.fc1 = nn.Linear(self.atten_embed_size, 1)
        if self.pred:
            self.fc2 = nn.Linear(self.atten_embed_size + 512, self.pred_size)
        self.eps = eps
