...
class DTmodel(nn.Module): # Transformer our model v1.0
    def __init__(
        ...
        self.stat_vocab_size = data_config.cond_vocab_size
        ...
        self.lab_vocab_size = data_config.lab_vocab_size
        ...
        if self.lab_vocab_size:
            self.lab = nn.Linear(self.lab_vocab_size, self.latent_size)
            self.atten_embed_size += self.latent_size
        ...
        self.adapter = nn.Linear(
            self.latent_size * 4 + self.cross_embed_size, self.atten_embed_size
        )
    ...
    def set_mean_std(self, mean, std):
        ...
        self.lab_mean = mean["lab"]
        self.lab_std = std["lab"]

        self.stat_mean = mean["stat"]
        self.stat_std = std["stat"]

    def forward(self, meds, chart, out, proc, lab, stat, demo):
        ...
        if self.proc_vocab_size:
            ...
            if out1.nelement():
                ...
                out1 = procEmbedded
        if self.lab_vocab_size:
            lab = (lab - self.lab_mean) / (self.lab_std + self.eps)
        ...
        out1 = self.atten0(out1, out1, out1)
        ...
        out = self.cross_attn(out2, out1, out1).squeeze(1)
...
class TransformerBlock(nn.Module):
    def __init__(self, embed_size, heads=8, dropout=0.1, forward_expansion=4):
        ...
        self.attention = nn.MultiheadAttention(
            embed_dim=embed_size, num_heads=heads, dropout=dropout
        )
