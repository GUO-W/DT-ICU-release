...
def test(model, test_loader):
    ...
    for step, batch in enumerate(tqdm(test_loader)):
        ...
        while meds.shape[1] < max_pred_length and not stop_pred:
            ...
            output_, _, _, _ = model(
                meds, chart, out, proc, date, ing/10000, stat, demo, None
            )
            ...
            print("pred prob overinjection:", output_.squeeze().item())
        ...
        for i in range(len(input_list)):
            if i not in skip_indexes:
                print(batch[i].shape)
                print(input_list[i][:, input_min_length:input_min_length + precision_min_length].mean(), input_list[i][:, input_min_length:input_min_length + precision_min_length].max(), input_list[i][:, input_min_length:input_min_length + precision_min_length].min())
        ...
        precision = precision.item() / (len(input_list) - len(skip_indexes))
