...
def load_data(mode, ids):
    file_paths = [cfg.root_dir + "/data/mimiciv3.1/processed_icu/" + str(i[0]) + "_" + str(i[1]) for i in ids]
...
def process_files(args):  # Function to process a single file
    ...
    label_dict = labels.set_index(labels.columns[2])[labels.columns[3]].to_dict()
    nb_neg = 0
    nb_pos = 0
    ...
    for file_path in tqdm(file_paths):
        ...
        if label == 1:
            ...
            if nb_neg > nb_pos:
                #print("skip pos sample to keep data balance", file_path)
                continue
            else:
                nb_neg += 1
    ...
    print("Pos sample:", nb_pos, "Neg sample:", nb_neg)
    return infos
