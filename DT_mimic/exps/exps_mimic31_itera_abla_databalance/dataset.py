    ...
    def __iter__(self):
        if self.mode == "train":
            if self.balance_type == "original":
                ...
                all_indices = self.buckets[0][0] + self.buckets[0][1] + self.buckets[1][0] + self.buckets[1][1]
                for i in range(0, self.epoch_size * self.batch_size, self.batch_size):
                    yield all_indices[i : i + self.batch_size]

            elif self.balance_type == "only_label_balance":
                draw = self.rng.choices   # alias
                combs = [[0], [1]]
                for idx in range(self.epoch_size):
                    choice = idx % len(combs)
                    comb = combs[choice]
                    pool = self.buckets[0][comb[0]] + self.buckets[1][comb[0]]
                ...
                for idx in range(self.epoch_size):
                    choice = idx % len(combs)
                    comb = combs[choice]
                    pool = self.buckets[comb[0]][0] + self.buckets[comb[0]][1]
...
def build_dataloaders(train_dataset, val_dataset, test_dataset, data_config, train_ids, val_ids, test_ids):
    ...
    train_batch_sampler = LengthAwareBalancedBatchSampler(train_seq_lengths, train_labels, [cfg.long_short_threshold], cfg.batch_size, mode="train", balance_type="only_label_balance")
