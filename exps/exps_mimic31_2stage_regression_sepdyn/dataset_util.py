...
if __name__ == "__main__":
    ...
    for f in files[:1000]:
        ...
        cols_dict1 = get_cols(dyn, 1)
        chart = dyn[cols_dict1["chartevents_checkbox"]].to_numpy()
        data = dyn[cols_dict1["datetimeevents_datetime"]].to_numpy()
        output = dyn[cols_dict1["outputevents_datetime"]].to_numpy()
        print("chartevents_checkbox", chart.min(), chart.max(), chart.mean())
        print("datetimeevents_datetime", data.min(), data.max(), data.mean())
        print("outputevents_datetime", output.min(), output.max(), output.mean())
