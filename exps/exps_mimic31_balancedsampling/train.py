...
from utils import fix_seed, get_data_mean_std_new
...
def build_model(datacfg):
    ...
    f1_loss = SoftF1Loss()
    l2_loss = nn.MSELoss()
    return model, evaluation, optimizer, bce_loss, f1_loss, l2_loss
...
def train(model, evaluation, optimizer, bce_loss, f1_loss, l2_loss, datacfg, train_loader, val_loader, test_loader, grad_accum_steps):
        ...
        for step, batch in enumerate(tqdm(train_loader)):
            ...
            if cfg.model.pred:
                output, logits, preds = model(
                    meds[:, :-1], chart[:, :-1], out[:, :-1], proc[:, :-1], date[:, :-1], ing[:, :-1], stat_train, demo_train, key_padding_mask
                )
...
def val(model, val_loader, evaluation):
    ...
    for batch in tqdm(val_loader):
        ...
        if cfg.model.pred:
            output, logits, preds = model(
                meds[:, :-1], chart[:, :-1], out[:, :-1], proc[:, :-1], date[:, :-1], ing[:, :-1], stat, demo, key_padding_mask
            )
            gt_preds = [
                meds[:, -1:],
                chart[:, -1:],
...
if __name__ == "__main__":
    ...
    model, evaluation, optimizer, bce_loss, f1_loss, l2_loss = build_model(datacfg=datacfg)

    # means, stds = get_data_mean_std_new(train_dataset, val_dataset)
    # model.set_mean_std(means, stds)

    train(model, evaluation, optimizer, bce_loss, f1_loss, l2_loss, datacfg, train_loader, val_loader, test_loader, cfg.grad_accum_steps)
